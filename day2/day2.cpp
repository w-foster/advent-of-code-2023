...
int main() {
    ...
    std::unordered_map<std::string, int> max_frequencies = {
        {"red", 12},
        {"green", 13},
        {"blue", 14}
    };
    ...
    for (auto &line : colours_by_round_by_line) {
        std::cout << "Game " << game << ": \n";
        bool possible = true; // assume that the round is possible, for potential counterexample
        for (auto &round : line) {
            std::cout << "Round: " << "\n";
